...
/* Globals/Public *************************************************************/
//LinkList head=NULL;//jacky
//pthread_t queThread;//jacky
//int msgId;//jacky
...
//add by jacky send conflict rule ids to messagequeue
/*void * sendDataToQue(void *a){
	while(true)
	{
		LinkList p = head;
		int i;
		while (p->next != NULL) {
			...
			printf("conflict rule ids: ");
			for (i = 0; i < 10; i++) {
				if (p->data.value[i] != 0) {
					printf("%ld,", p->data.value[i]);
				}
			}
			printf("\n");
			printf("------------------------------");
...
int SnortMain(int argc, char *argv[])
{
 ...
 // head=InitiateLinkList();//jacky
 /* if((msgId=msgget((key_t)123456,IPC_CREAT|0666))==-1){
             	                    printf("fail to create msgget:");
             	                }*/
  //msgId=CreateMessage((key_t)123456);


  //msgId=CreateMessage((key_t)123456);//jacky create messageque
  //printf("msgId=%d\n",msgId);
  /*int err = pthread_create(&queThread,NULL,sendDataToQue,NULL);
  	 if(err!=0)
  	 {

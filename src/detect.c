...
int Detect(Packet * p)
{
	...
	double after=0;
	gettimeofday(&t, 0);
	before=t.tv_sec + 1E-6 * t.tv_usec;
  ...
  /*  SessionControlBlock *scb = NULL;
    scb = (SessionControlBlock *) (p->ssnptr);
    //printf("before scb->flagAlert=%d\n",scb->flagAlert);
    if(detected)
    scb->flagAlert=detected;*/

    //printf("detected=%d,sp=%d,dp=%d\n",detected,p->sp,p->dp);
    /*******zhbit jacky****************/
    ...
    /************zhbit jacky**********/
    	...
    	gettimeofday(&t, 0);
    	after=t.tv_sec + 1E-6 * t.tv_usec;
    	p->spanTime=after-before;

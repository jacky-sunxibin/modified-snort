...
#include "session_common.h" //jacky
/*#include "profiler.h"//jacky*/

/*#include<sys/types.h>  //jacky
#include<sys/ipc.h>
#include<sys/msg.h>   //jacky

#include "LinkList.h" //jacky
...
/*
 ...
 #include <arpa/inet.h> //jacky
*/

//#include "MessageQueue.h"  //jacky
/*
...
struct msgbuf{
        ...
        long mtext[_MAX_BUFFER];//发送的消息(长度、类型可以自行指定)
}; //jacky
*/
...
int fpAddMatch(OTNX_MATCH_DATA *omd_local, int pLen, OptTreeNode *otn)
{
    ...
    //modify by jacky
     evalIndex =((struct _ListHead*)rtn->listhead)->ruleListNode->evalIndex;
...
static inline int fpFinalSelectEvent(OTNX_MATCH_DATA *o, Packet *p)
{
    ...
    OptTreeNode *otn;
    int tcnt = 0;
    static int tempCount;//jacky
    static int tempNoTrackConnectionCount; //jacky
    //static double totalTime; //jacky
   // int msgQueId=0;//jacky
    ...
    for( i = 0; i < o->iMatchInfoArraySize; i++ )
    {
        ...
        if(o->matchInfo[i].iMatchCount)
        {
            ...
            for(j=0; j < o->matchInfo[i].iMatchCount; j++)
            {
                ...
                //tempCount++;
               //printf("the %dth (attack) zhbit sid=%d,message=%s,sp=%d,dp=%d,imatchCount=%d seq=%x,time=%ld\n",tempCount,otn->sigInfo.id,otn->sigInfo.message,p->sp,p->dp,o->matchInfo[i].iMatchCount,p->tcph->th_seq,p->pkth->ts.tv_usec);
                /**********zhbit jacky**************/
//                printf("zhbit sid=%d,message=%s\n",otn->sigInfo.id,otn->sigInfo.message);
//                printf("zhbit fragment count=%d\n",p->overlapCount);
                ///printf("zhbit sid=%d,message=%s\n",otn->sigInfo.id,otn->sigInfo.message);
                //printf("zhbit fragment count=%d\n",p->overlapCount);
               // buf.mtext[j]=otn->sigInfo.id;
                //x.value[j]=otn->sigInfo.id;
                //printf("zhbit sid=%d,message=%s\n",otn->sigInfo.id,otn->sigInfo.message);
               // printf("zhbit sid=%d,message=%s\n",otn->sigInfo.id,otn->sigInfo.message);
                /**********zhbit jacky**************/
                ...
                */
                   ...
                   !fpFragAlerted(p, otn))
                {
                	...
                	/********* Rewrite output module  add code  by jacky************/
                	/*SessionControlBlock *scb;//zhbit jacky
					...
					if(scb!=NULL){
						//tempNoTrackConnectionCount++;
						//totalTime+=p->spanTime;
						if(scb->flagTrackConnection){
							if(!scb->flagAlert){
								tempNoTrackConnectionCount++;
								printf("the %dth,sid=%d,message=%s, sip=%s,dip=%s\n",tempNoTrackConnectionCount,otn->sigInfo.id,otn->sigInfo.message, srcP,dstP);
                        ...
                        if(scb->flagTrackConnection){
							...
							if (!scb->flagAlert) {
								tempCount++;
									char *srcP = SnortStrdup(
											inet_ntoa(&scb->origin_client_ip));
									char *dstP = SnortStrdup(
											inet_ntoa(&scb->origin_server_ip));
									if (strstr(srcP, "192.168.5") != NULL
											|| strstr(srcP, "192.168.6") != NULL) {
										printf(
												" the %dth (normal) sid=%d,message=%s, sip=%s,dip=%s\n",
												tempCount, otn->sigInfo.id,
												otn->sigInfo.message, srcP,
												dstP);
									} else {
										printf(
												" the %dth (attack) sid=%d,message=%s, sip=%s,dip=%s\n",
												tempCount, otn->sigInfo.id,
												otn->sigInfo.message, srcP,
												dstP);
									}
									...
									char *dstP = SnortStrdup(
											inet_ntoa(&scb->origin_server_ip));
									//if (p->sp >= 16651 && p->sp <= 16900)
									if (p->sp >= 15000 && p->sp <= 16900) {
										printf(
												"the %dth (normal) sid=%d,message=%s, sp=%d,dp=%d, sip=%s, dip=%s\n",
												tempCount, otn->sigInfo.id,
												otn->sigInfo.message, p->sp,
												p->dp, srcP, dstP);
										...
										printf(
												"the %dth (attack) sid=%d,message=%s, sp=%d,dp=%d\n",
												tempCount, otn->sigInfo.id,
												otn->sigInfo.message, p->sp,
												p->dp);
                        	...
                        	char *dstP = SnortStrdup(inet_ntoa(&p->ip4h->ip_dst));
                       	    printf("(attack) the %dth sid=%d,message=%s srcP=%s,sp=%d, dstP=%s,dp=%d,seq=%x,time=%ld\n",tempNoTrackConnectionCount,otn->sigInfo.id,otn->sigInfo.message,srcP,p->sp,dstP,p->dp,p->tcph->th_seq,p->pkth->ts.tv_usec);
                        }
						...
						tempNoTrackConnectionCount++;
						printf("the %dth,(attack) sid=%d,message=%s,srcP=%s,dstP=%s\n",tempNoTrackConnectionCount,otn->sigInfo.id,otn->sigInfo.message,srcP,dstP);

						char *srcP = SnortStrdup(inet_ntoa(&p->ip4h->ip_src));
						char *dstP = SnortStrdup(inet_ntoa(&p->ip4h->ip_dst));
						if(strstr(srcP,"172.16.112.21")!=NULL){
							printf("(normal) sid=%d,message=%s,srcP=%s,dstP=%s\n",otn->sigInfo.id,otn->sigInfo.message,srcP,dstP);
						}else{
							printf("(attack) sid=%d,message=%s,srcP=%s,dstP=%s\n",otn->sigInfo.id,otn->sigInfo.message,srcP,dstP);
						}
...
*/
               ...
               /********* add code  by jacky************/
					...
					if (scb != NULL) {
						if (scb->flagTrackConnection) {
							if (!scb->flagAlert) {
								int err = SnortEventqAdd(otn->sigInfo.generator,
										otn->sigInfo.id, otn->sigInfo.rev,
										otn->sigInfo.class_id,
										otn->sigInfo.priority,
										otn->sigInfo.message, (void *) otn);
								scb->flagAlert = 1;
								if (err)
									pc.queue_limit++;
							...
							int err = SnortEventqAdd(otn->sigInfo.generator,
									otn->sigInfo.id, otn->sigInfo.rev,
									otn->sigInfo.class_id,
									otn->sigInfo.priority, otn->sigInfo.message,
									(void *) otn);
						...
						int err = SnortEventqAdd(otn->sigInfo.generator,
								otn->sigInfo.id, otn->sigInfo.rev,
								otn->sigInfo.class_id, otn->sigInfo.priority,
								otn->sigInfo.message, (void *) otn);
           ...
           /* printf("\n-----end----\n");//jacky
            if(x.value[0]!=0)
            AddLinkList(head,x);*/
           /* if(o->matchInfo[i].iMatchCount>0)
            {
            	 if(msgsnd(msgQueId,&buf,sizeof(buf.mtext),-1) == -1){
            	                    perror("msgsnd");
            	                }
            }
            printf("\n");*/
...
int fpEvalPacket(Packet *p)
{
    ...
    switch(ip_proto)
    {
        ...
        case IPPROTO_ICMP:
           ...
           /* int temp=fpEvalHeaderIcmp(p, omd);
            printf("zhbit=%d\n",omd->matchInfo->);
            return temp;*/

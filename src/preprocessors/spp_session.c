...
/*******************zhbit jacky*************************/
static void countBytes(SessionControlBlock *scb,Packet *p);
static void initSCB(SessionControlBlock *scb,Packet *p);
static int getInternalState(SessionControlBlock *scb);
static int isSerror(SessionControlBlock *scb);
static int isRerror(SessionControlBlock *scb);
static boolean isExist(SessionControlBlock* arr[], SessionControlBlock* scb);
static void setBeforeNoTCP100SessionStatistics(void *sessionCache,SessionControlBlock *scb);
static int findLast100TcpSession(void *sessionCache,SessionControlBlock *scb,SessionControlBlock* beforeSessions[]);
static void clearBefore100SecondsTCPSessonError(SessionControlBlock *scb);
static void setBefore100TCPSessionErrorStatistics(void *sessionCache,SessionControlBlock *scb);
static void setBefore100TCPSessionStatistics(void *sessionCache,SessionControlBlock *scb);
static void setBefore2SecondsTcpSessionStatistics(void *sessionCache,SessionControlBlock *scb);
static void clearBefore2SecondsTCPSessonError(SessionControlBlock *scb);
static void setBefore2SecondsTCPSessionErrorStatistics(void *sessionCache,SessionControlBlock *scb);
static void setTcpSessionStatistics(void *sessionCache,SessionControlBlock *scb);
/*******************zhbit jacky*************************/
...
static SessionConfiguration *initSessionConfiguration( void )
{
    ...
    sessionConfig->track_icmp_sessions = STREAM_TRACK_YES;//modify by zhbit jacky from NO to YES
...
/************zhbit jacky***************/
static int getInternalState(SessionControlBlock *scb){
	switch(scb->sessionState){
	case SESSION_STATE_ESTAB:
		return SESSION_STATE_S1;
		break;
	case SESSION_STATE_S4:
			return SESSION_STATE_OTH;
			break;
	case SESSION_STATE_S2F:
			return SESSION_STATE_S2;
			break;
	case SESSION_STATE_S3F:
			return SESSION_STATE_S3;
			break;
	default:
		return scb->sessionState;
		break;
	}
	return scb->sessionState;
}
static int isSerror(SessionControlBlock *scb){
	int state=getInternalState(scb);
	switch(state){
		case SESSION_STATE_S0:
		case SESSION_STATE_S1:
		case SESSION_STATE_S2:
		case SESSION_STATE_S3:
		case SESSION_STATE_RSTR:
		     return true;
		     break;
		default:
			break;
...
static int isRerror(SessionControlBlock *scb){
      return getInternalState(scb)==SESSION_STATE_REJ;
}

static void printfSessionStaticsInfo(void *sessionCache){
	SFXHASH * t;
	SFXHASH_NODE * n;
	SessionCache *session_cache = (SessionCache *) sessionCache;
	t = session_cache->hashTable;
	printf("\n********start update**********\n");
	for (n = sfxhash_findfirst(t); n != 0; n = sfxhash_findnext(t)) {
		SessionControlBlock *temp = n->data;
		/*if (temp == scb)
		 continue;*/
		char *srcP = SnortStrdup(inet_ntoa(&temp->origin_client_ip));
		char *dstP = SnortStrdup(inet_ntoa(&temp->origin_server_ip));

		printf("\nsip=%s,dip=%s,srcPort=%d,dstPort=%d\n", srcP, dstP,
				temp->origin_client_port, temp->origin_server_port);
		printf(
				"\nfeature value is: %ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld\n",
				temp->two_seconds_same_host, temp->two_seconds_same_server,
				temp->two_seconds_same_host_syn_error,
				temp->two_seconds_same_server_syn_error,
				temp->two_seconds_same_host_rej_error,
				temp->two_seconds_same_server_rej_error,
				temp->two_seconds_same_host_and_server,
				temp->two_seconds_same_host
						- temp->two_seconds_same_host_and_server,
				temp->two_seconds_same_server
						- temp->two_seconds_same_host_and_server,
				temp->before_100_same_host,
				temp->before_100_same_host_and_server,
				temp->before_100_same_host_and_server,
				temp->before_100_same_host_differ_server,
				temp->before_100_same_host_same_source_port,
				temp->before_100_same_host_and_server_differ_source_ip,
				temp->before_100_same_host_syn_error,
				temp->before_100_same_host_and_server_syn_error,
				temp->before_100_same_host_rej_error,
				temp->before_100_same_host_and_server_rej_error);
...
//static void updateTCP100SessionError(void *sessionCache,SessionControlBlock *scb,int flagSyn,int flagRej,int flag ){
//	SFXHASH * t;
//	SFXHASH_NODE * n;
//	int i;
//	SessionCache *session_cache = (SessionCache *) sessionCache;
//	t = session_cache->hashTable;
//	SessionControlBlock* afterSessions[100];
//	for (i = 0; i < 100; i++) {
//		afterSessions[i] = NULL;
//	}
//	int length = sfghash_count(t);
//	int count = 0;
//	for (i = 0; i < length; i++) {
//		if (count >= 100)
//			break;
//		long minSpanTime = 9999999999999;
//		SessionControlBlock* minScb = NULL;
//		for (n = sfxhash_findfirst(t); n != 0; n = sfxhash_findnext(t)) {
//			SessionControlBlock* temp = n->data;
//			if (scb == temp || scb->first_data_seen > temp->first_data_seen
//					|| isExist(afterSessions, temp))
//				continue;
//			if (temp->first_data_seen - scb->first_data_seen < minSpanTime) {
//				minSpanTime = temp->first_data_seen - scb->first_data_seen;
//				minScb = temp;
//			}
//		}
//		if (minScb != NULL)
//			afterSessions[count++] = minScb;
//	}
//
//	for (i = 0; i < count; i++) {
//		SessionControlBlock* temp = afterSessions[i];
//		if (sfip_compare(&scb->origin_server_ip, &temp->origin_server_ip)
//				== SFIP_EQUAL) {
//			if (scb->origin_server_port == temp->origin_server_port) {
//				if (flag) {
//					if (flagSyn) {
//						temp->before_100_same_host_and_server_syn_error++;
//					} else {
//						if (temp->before_100_same_host_and_server_syn_error
//								> 0) {
//							temp->before_100_same_host_and_server_syn_error--;
//						}
//					}
...
//						temp->before_100_same_host_and_server_rej_error++;
//					} else {
//						if (temp->before_100_same_host_and_server_rej_error
//								> 0) {
//							temp->before_100_same_host_and_server_rej_error--;
//						}
//					}
...
//					temp->before_100_same_host_syn_error++;
//				} else {
//					if (temp->before_100_same_host_syn_error > 0) {
//						temp->before_100_same_host_syn_error--;
//					}
//				}
//			} else {
...
//					temp->before_100_same_host_rej_error++;
//				} else {
//					if (temp->before_100_same_host_rej_error > 0) {
//						temp->before_100_same_host_rej_error--;
//					}
//				}
//			}
...
//static void updateTcpSessionError(void *sessionCache, SessionControlBlock *scb,
//		int flag) {
//	SFXHASH * t;
//	SFXHASH_NODE * n;
//	int flagSyn = false;
//	int flagRej = false;
//	flagSyn = isSerror(scb);
//	flagRej = isRerror(scb);
//	SessionCache *session_cache = (SessionCache *) sessionCache;
//	t = session_cache->hashTable;
//	for (n = sfxhash_findfirst(t); n != 0; n = sfxhash_findnext(t)) {
//		SessionControlBlock *temp = n->data;
//		if (temp == scb || temp->first_data_seen < scb->first_data_seen)
//			continue;
//		if ((temp->first_data_seen - scb->first_data_seen) <= 2 * 1000000) {
//			if (sfip_compare(&scb->origin_server_ip, &temp->origin_server_ip)
//					== SFIP_EQUAL) {
//				if (flag) {
//					if (flagSyn) {
//						temp->two_seconds_same_host_syn_error++;
//					} else {
//						if (temp->two_seconds_same_host_syn_error > 0) {
//							temp->two_seconds_same_host_syn_error--;
//						}
//					}
//
...
//						temp->two_seconds_same_host_rej_error++;
//					} else {
//						if (temp->two_seconds_same_host_rej_error > 0) {
//							temp->two_seconds_same_host_rej_error--;
//						}
//					}
//				}
...
//						temp->two_seconds_same_server_syn_error++;
//					} else {
//						if (temp->two_seconds_same_server_syn_error > 0) {
//							temp->two_seconds_same_server_syn_error--;
//						}
//					}
//				} else {
...
//						temp->two_seconds_same_server_rej_error++;
//					} else {
//						if (temp->two_seconds_same_server_rej_error > 0) {
//							temp->two_seconds_same_server_rej_error--;
//						}
//					}
//				}
...
//		}
//	}
//	updateTCP100SessionError(sessionCache, scb, flagSyn, flagRej, flag);
//}
static void setNoTcpSessionStatistics(void *sessionCache,SessionControlBlock *scb){
	SFXHASH * t;
	SFXHASH_NODE * n;
	SessionCache *session_cache = (SessionCache *) sessionCache;
	t = session_cache->hashTable;
	...
	for (n = sfxhash_findfirst(t); n != 0; n = sfxhash_findnext(t)) {
		...
		if (temp == scb)
			continue;
		count++;
		if ((scb->first_data_seen > temp->first_data_seen)) {
			if ((scb->first_data_seen - temp->first_data_seen) <= 2 * 1000000) {
				if (sfip_compare(&scb->origin_server_ip,
						&temp->origin_server_ip) == SFIP_EQUAL) {
					scb->two_seconds_same_host++;
					if (scb->origin_server_port == temp->origin_server_port) {
						scb->two_seconds_same_host_and_server++;
					}
				}
				if (scb->origin_server_port == temp->origin_server_port) {
					scb->two_seconds_same_server++;
				}
			}
		}

	}
	setBeforeNoTCP100SessionStatistics(sessionCache,scb);
}
static boolean isExist(SessionControlBlock* arr[], SessionControlBlock* scb){
...
static void setBeforeNoTCP100SessionStatistics(void *sessionCache,SessionControlBlock *scb){
	...
	SessionCache *session_cache = (SessionCache *) sessionCache;
	t = session_cache->hashTable;
	SessionControlBlock* beforeSessions[100];
	for (i = 0; i < 100; i++) {
		beforeSessions[i] = NULL;
	}
	int length=sfghash_count(t);
	int count=0;
	for (i = 0; i < length; i++){
		...
		for (n = sfxhash_findfirst(t); n != 0; n = sfxhash_findnext(t)){
			SessionControlBlock* temp = n->data;
			if (scb == temp || scb->first_data_seen < temp->first_data_seen || isExist(beforeSessions,temp))
				continue;
			if(scb->first_data_seen - temp->first_data_seen
					< minSpanTime){
				minSpanTime=scb->first_data_seen - temp->first_data_seen;
				minScb=temp;
			}
		}
		if(minScb!=NULL)
		beforeSessions[count++]=minScb;
	}

	for (i = 0; i < count; i++) {
		SessionControlBlock* temp = beforeSessions[i];
		if (sfip_compare(&scb->origin_server_ip, &temp->origin_server_ip)
				== SFIP_EQUAL) {
			scb->before_100_same_host++;
			if (scb->origin_server_port == temp->origin_server_port) {
				scb->before_100_same_host_and_server++;
				if (sfip_compare(&scb->origin_client_ip,
						&temp->origin_client_ip) != SFIP_EQUAL) {
					scb->before_100_same_host_and_server_differ_source_ip++;
				}
			} else {
				scb->before_100_same_host_differ_server++;
			}
			if (scb->origin_client_port == temp->origin_client_port) {
				scb->before_100_same_host_same_source_port++;
			}
		}
		if (scb->origin_server_port == temp->origin_server_port) {
			scb->before_100_same_server++;
		}
	}
}
static int findLast100TcpSession(void *sessionCache,SessionControlBlock *scb,SessionControlBlock* beforeSessions[])
{
	...
	for (i = 0; i < length; i++) {
		...
		for (n = sfxhash_findfirst(t); n != 0; n = sfxhash_findnext(t)) {
			SessionControlBlock* temp = n->data;
			if (scb == temp || scb->first_data_seen < temp->first_data_seen
					|| isExist(beforeSessions, temp))
				continue;
			if (scb->first_data_seen - temp->first_data_seen < minSpanTime) {
				minSpanTime = scb->first_data_seen - temp->first_data_seen;
				minScb = temp;
			}
		}
...
static void clearBefore100SecondsTCPSessonError(SessionControlBlock *scb){
	scb->before_100_same_host_and_server_syn_error=0;
	scb->before_100_same_host_and_server_rej_error=0;
	scb->before_100_same_host_syn_error=0;
	scb->before_100_same_host_rej_error=0;
}
static void setBefore100TCPSessionErrorStatistics(void *sessionCache,SessionControlBlock *scb){
	...
	int count = findLast100TcpSession(sessionCache, scb, beforeSessions);
//	printf("\n********start*************\n");
//	for(i=0;i<count;i++){
//		printf("clientPort=%d,",beforeSessions[i]->origin_client_port);
//	}
//	printf("\n********end*************\n");
	clearBefore100SecondsTCPSessonError(scb);
	for (i = 0; i < count; i++) {
		SessionControlBlock* temp = beforeSessions[i];
//		char *scbClientPort = SnortStrdup(inet_ntoa(&scb->origin_client_ip));
//		printf("clientPort=%d,",beforeSessions[i]->origin_client_port);
		//int result=sfip_compare(&scb->origin_server_ip, &temp->origin_server_ip);
		//printf("scb->origin_server_port=%d,temp->origin_server_port=%d,result=%d isSerror=%d,state=%d\n",scb->origin_server_port,temp->origin_server_port,result,isSerror(temp),temp->sessionState);

		if (sfip_compare(&scb->origin_server_ip, &temp->origin_server_ip)
				== SFIP_EQUAL) {
			if (scb->origin_server_port == temp->origin_server_port) {
				if (isSerror(temp)) {
					scb->before_100_same_host_and_server_syn_error++;
					//printf("before_100_same_host_and_server_syn_error=%d,clientPort=%d\n",scb->before_100_same_host_and_server_syn_error,scb->origin_client_port);
				} else
				if (isRerror(temp)) {
					scb->before_100_same_host_and_server_rej_error++;
				}
			}
			if (isSerror(temp)) {
				scb->before_100_same_host_syn_error++;
			}
			if (isRerror(temp)) {
...
//static void setBeforeTCP100StatisticSession(void *sessionCache,SessionControlBlock *scb){
//	SFXHASH * t;
//    SFXHASH_NODE * n;
//    int i;
//	SessionCache *session_cache = (SessionCache *) sessionCache;
...
//					scb->before_100_same_host_and_server_syn_error++;
//				} else {
//					if (scb->before_100_same_host_and_server_syn_error > 0) {
//						//scb->before_100_same_host_and_server_syn_error--;
//					}
//				}
//				if (isRerror(temp)) {
//					scb->before_100_same_host_and_server_rej_error++;
//				} else {
//					if (scb->before_100_same_host_and_server_rej_error > 0) {
//						//scb->before_100_same_host_and_server_rej_error--;
//					}
//				}
//			} else {
...
//				scb->before_100_same_host_syn_error++;
//			} else {
//				if (scb->before_100_same_host_syn_error > 0) {
//					//scb->before_100_same_host_syn_error--;
//				}
//			}
//			if (isRerror(temp)) {
//				scb->before_100_same_host_rej_error++;
//			} else {
//				if (scb->before_100_same_host_rej_error > 0) {
//					//scb->before_100_same_host_rej_error--;
//				}
//			}
//		}
...
static void setBefore2SecondsTcpSessionStatistics(void *sessionCache,SessionControlBlock *scb){
	...
	for (n = sfxhash_findfirst(t); n != 0; n = sfxhash_findnext(t)) {
		SessionControlBlock *temp = n->data;
		if (temp == scb || scb->first_data_seen < temp->first_data_seen) {
			continue;
		}
		if ((scb->first_data_seen - temp->first_data_seen) <= 2 * 1000000) {
			if (sfip_compare(&scb->origin_server_ip, &temp->origin_server_ip)
...
static void clearBefore2SecondsTCPSessonError(SessionControlBlock *scb){
	scb->two_seconds_same_host_syn_error=0;
	scb->two_seconds_same_host_rej_error=0;
	scb->two_seconds_same_server_syn_error=0;
	scb->two_seconds_same_server_rej_error=0;
}
static void setBefore2SecondsTCPSessionErrorStatistics(void *sessionCache,SessionControlBlock *scb){
	...
	SFXHASH * t;
		SFXHASH_NODE * n;
		SessionCache *session_cache = (SessionCache *) sessionCache;
		clearBefore2SecondsTCPSessonError(scb);
		t = session_cache->hashTable;
	for (n = sfxhash_findfirst(t); n != 0; n = sfxhash_findnext(t)) {
		SessionControlBlock *temp = n->data;
		if (temp == scb || scb->first_data_seen < temp->first_data_seen) {
		...
		if ((scb->first_data_seen - temp->first_data_seen) <= 2 * 1000000) {
			if (sfip_compare(&scb->origin_server_ip, &temp->origin_server_ip)
					== SFIP_EQUAL) {
				if (isSerror(temp)) {
					scb->two_seconds_same_host_syn_error++;
				}
				if (isRerror(temp)) {
					scb->two_seconds_same_host_rej_error++;
				}
			}
			if (scb->origin_server_port == temp->origin_server_port) {
				if (isSerror(temp)) {
					scb->two_seconds_same_server_syn_error++;
				}
				if (isRerror(temp)) {
					scb->two_seconds_same_server_rej_error++;
				}
			}
		}
...
static void setTcpSessionStatistics(void *sessionCache,SessionControlBlock *scb){
	setBefore2SecondsTcpSessionStatistics(sessionCache,scb);
	setBefore2SecondsTCPSessionErrorStatistics(sessionCache,scb);
	setBefore100TCPSessionStatistics(sessionCache,scb);
	setBefore100TCPSessionErrorStatistics(sessionCache,scb);

//	SFXHASH * t;
//	SFXHASH_NODE * n;
//	SessionCache *session_cache = (SessionCache *) sessionCache;
//	t = session_cache->hashTable;
//	      /*  char *srcP1 = SnortStrdup(inet_ntoa(&scb->origin_client_ip));
//			char *dstP1 = SnortStrdup(inet_ntoa(&scb->origin_server_ip));
//			printf("scbSIP=%s,scbDIP=%s,scbsrcPort=%ld,scbdstPort=%ld\n", srcP1, dstP1,
//					scb->origin_client_port, scb->origin_server_port);*/
//	 //printf("\n**********start******************\n");
//	for (n = sfxhash_findfirst(t); n != 0; n = sfxhash_findnext(t)) {
//		SessionControlBlock *temp = n->data;
//		if(temp==scb || scb->first_data_seen< temp->first_data_seen)
...
//		/*	char *srcP = SnortStrdup(inet_ntoa(&temp->origin_client_ip));
//					char *dstP = SnortStrdup(inet_ntoa(&temp->origin_server_ip));
//					printf("tempSIP=%s,tempDIP=%s,tempsrcPort=%d,tempdstPort=%d, synError=%d\n", srcP, dstP,
//						temp->origin_client_port, temp->origin_server_port,isSerror(temp));
//		*/
//
//
//		  if ((scb->first_data_seen - temp->first_data_seen) <= 2 * 1000000) {
...
//					scb->two_seconds_same_host_syn_error++;
//				} else {
//					if (scb->two_seconds_same_host_syn_error > 0) {
//						//scb->two_seconds_same_host_syn_error--;
//					}
//				}
//				if (isRerror(temp)) {
//					scb->two_seconds_same_host_rej_error++;
//				} else {
//					if (scb->two_seconds_same_host_rej_error > 0) {
//						//scb->two_seconds_same_host_rej_error--;
//					}
//				}
//				if (scb->origin_server_port == temp->origin_server_port) {
...
//					scb->two_seconds_same_server_syn_error++;
//				} else {
//					if (scb->two_seconds_same_server_syn_error > 0) {
//						//scb->two_seconds_same_server_syn_error--;
//					}
//				}
//				if (isRerror(temp)) {
//					scb->two_seconds_same_server_rej_error++;
//				} else {
//					if (scb->two_seconds_same_server_rej_error > 0) {
//						//scb->two_seconds_same_server_rej_error--;
//					}
//				}
//			}
...
//	}
//
//	setBeforeTCP100StatisticSession(sessionCache,scb);
	/*if(scb->origin_client_port==8055){
		printf("scb syn error=%d\n",scb->before_100_same_host_and_server_syn_error);
		printf("\n**********end******************\n");
	}*/

//	printf("\n**********end******************\n");

	//printf("before_100_same_host_and_server=%d\n",scb->before_100_same_host_and_server);

	//printf("\n**********end******************\n");
}

static void updateTcpSessionState(void *sessionCache,SessionControlBlock *scb,Packet *p){
	int originator = 0;
	if (IS_IP4(p)) {
		if (sfip_fast_eq4(&p->ip4h->ip_src, &scb->origin_client_ip)
				&& sfip_fast_eq4(&p->ip4h->ip_dst, &scb->origin_server_ip)) {
			originator = 1;
		}
	} else {
		if (sfip_fast_eq6(&p->ip6h->ip_src, &scb->origin_client_ip)
				&& sfip_fast_eq6(&p->ip6h->ip_dst, &scb->origin_server_ip)) {
			originator = 1;
		}
	}
...
//	printf("\n*******start**********\n");
//    int oldFlagSynError=isSerror(scb);
//    printf("before currentStat=%d,synError=%d\n",scb->sessionState,oldFlagSynError);

//    int oldFlagRejError=isRerror(scb);

	switch (scb->sessionState) {
	case SESSION_STATE_INIT:
        if(((p->tcph->th_flags&TH_SYN)!=0) && ((p->tcph->th_flags&TH_ACK)!=0) )
        	scb->sessionState=SESSION_STATE_S4;
        else if((p->tcph->th_flags & TH_SYN)!=0)
        	scb->sessionState=SESSION_STATE_S0;
        else
        	scb->sessionState=SESSION_STATE_OTH;
		break;
	case SESSION_STATE_S0:
		if(originator){
			if((p->tcph->th_flags&TH_RST)!=0)
				scb->sessionState=SESSION_STATE_RSTOS0;
			else if((p->tcph->th_flags&TH_FIN)!=0)
				scb->sessionState=SESSION_STATE_SH;
		}else{
			if ((p->tcph->th_flags & TH_RST)!= 0){
				scb->sessionState = SESSION_STATE_REJ;
				//printf("scb->sessionState=%d,scb->origin_client_port=%d\n",scb->sessionState,scb->origin_client_port);
			}

			else if (((p->tcph->th_flags&TH_SYN)!=0) && ((p->tcph->th_flags&TH_ACK)!=0) )
				scb->sessionState = SESSION_STATE_S1;
		}
		break;
	case SESSION_STATE_S4:
		if(originator){
			if((p->tcph->th_flags&TH_RST)!=0)
				scb->sessionState=SESSION_STATE_RSTRH;
			else if((p->tcph->th_flags&TH_FIN)!=0)
				scb->sessionState=SESSION_STATE_SHR;
		}
		break;
	case SESSION_STATE_S1:
		if (originator) {
			if ((p->tcph->th_flags & TH_RST) != 0)
				scb->sessionState = SESSION_STATE_RSTO;
			else if ((p->tcph->th_flags & TH_ACK) != 0)
				scb->sessionState = SESSION_STATE_ESTAB;
		} else {
			if ((p->tcph->th_flags & TH_RST) != 0)
				scb->sessionState = SESSION_STATE_RSTR;
		}
		break;
	case SESSION_STATE_ESTAB:
		if (originator) {
			if ((p->tcph->th_flags & TH_RST)!= 0)
				...
				scb->sessionState = SESSION_STATE_S2;
		} else {
			if ((p->tcph->th_flags & TH_RST)!= 0)
				scb->sessionState = SESSION_STATE_RSTR;
			else if ((p->tcph->th_flags & TH_FIN)!= 0)
				scb->sessionState = SESSION_STATE_S3;
		}
		break;
	case SESSION_STATE_S2:
		if (originator) {
			...
			if ((p->tcph->th_flags & TH_RST) != 0)
				...
				scb->sessionState = SESSION_STATE_S2F;
		}
		break;
	case SESSION_STATE_S3:
		if (originator) {
			if ((p->tcph->th_flags & TH_RST) != 0)
				...
				scb->sessionState=SESSION_STATE_S3F;
		} else {
			if ((p->tcph->th_flags & TH_RST) != 0)
				scb->sessionState = SESSION_STATE_RSTR;
		}
	...
	case SESSION_STATE_S2F:
			if (originator) {
				if ((p->tcph->th_flags & TH_RST) != 0)
					...
					scb->sessionState=SESSION_STATE_SF;
			} else {
				if ((p->tcph->th_flags & TH_RST) != 0)
					scb->sessionState = SESSION_STATE_RSTR;
			}
    ...
    //setBefore100TCPSessionErrorStatistics(sessionCache,scb);
    //setBefore2SecondsTCPSessionErrorStatistics(sessionCache,scb);
    //printf("scb->before_100_same_host_and_server_syn_error=%ld\n",scb->before_100_same_host_and_server_syn_error);

//	 int newFlagSynError=isSerror(scb);
//	 int newFlagRejError=isRerror(scb);
//	if((oldFlagSynError&&!newFlagSynError)||(!oldFlagSynError&&newFlagSynError))
//	{
//		updateTcpSessionError(sessionCache,scb,1);
//	}
//	if((oldFlagRejError&&!newFlagRejError)||(!oldFlagRejError&&newFlagRejError)){
//		updateTcpSessionError(sessionCache,scb,0);
//	}
	//printf("scb->sessionState=%ld\n",scb->sessionState);


//	    int newFlagSynError=isSerror(scb);
//	    printf("after currentStat=%d,synError=%d\n",scb->sessionState,newFlagSynError);
//	    printf("\n*******end**********\n");

}

void CountBefore100TCPSessionErrorStatistics(SessionControlBlock *scb){
	setBefore100TCPSessionErrorStatistics(proto_session_caches[ SESSION_PROTO_TCP ],scb);
}
void CountBefore2SecondsTCPSessionErrorStatistics(SessionControlBlock *scb){
	setBefore2SecondsTCPSessionErrorStatistics(proto_session_caches[ SESSION_PROTO_TCP ],scb);
}
    ...
    if( p->ssnptr == NULL )
    {
        switch( GET_IPH_PROTO( p ) )
        {
            case IPPROTO_TCP:
                ...
                updateTcpSessionState(proto_session_caches[ SESSION_PROTO_TCP ],scb,p);//zhbit jacky
                break;
            ...
            case IPPROTO_UDP:
                ...
                {
                	scb = createSession( proto_session_caches[ SESSION_PROTO_UDP ], p, &key ); //origin code
                	//char *srcP = SnortStrdup(inet_ntoa(&scb->origin_client_ip));
                	//char *dstP = SnortStrdup(inet_ntoa(&scb->origin_server_ip));
                	//printf("scb->origin_client_ip=%s,scb->origin_client_port=%ld,scb->origin_server_ip=%s,scb->origin_server_port=%ld\n",srcP,scb->origin_client_port,dstP,scb->origin_server_port);
                }
            ...
            case IPPROTO_ICMP:
               ...
               /* if( scb != NULL )
                    break;*/ //delete by jacky
                if(scb!=NULL){

                	/* char *srcP = SnortStrdup(inet_ntoa(&scb->origin_client_ip));
                	 char *dstP = SnortStrdup(inet_ntoa(&scb->origin_server_ip));
                printf("\n****************not empty star*****************\n");
                printf("pack=%s,scb=%s,sp=%d,dp=%d,count=%d\n", srcP, dstP, scb->origin_client_port,scb->origin_server_port,scb->two_seconds_same_host);
				 printf("\n****************not empty end*****************\n");*/
                	//printf("scb->origin_client_port=%ld,scb->scb->origin_server_port=%ld\n",scb->origin_client_port,scb->origin_server_port);
                	break;
                }
                ...
                {
                    scb = createSession( proto_session_caches[ SESSION_PROTO_ICMP ], p, &key ); //modify by jacky from IP to ICMP

                    //char *srcP = SnortStrdup(inet_ntoa(GET_SRC_IP(p)));&temp->origin_client_ip
                   /* char *srcP = SnortStrdup(inet_ntoa(&scb->origin_client_ip));
                    char *dstP = SnortStrdup(inet_ntoa(&scb->origin_server_ip));
				printf("\n**************** new star*****************\n");
				printf("pack=%s,scb=%s,sp=%d,dp=%d,count=%d\n", srcP, dstP, scb->origin_client_port,scb->origin_server_port,scb->two_seconds_same_host);
...
static void countBytes(SessionControlBlock *scb,Packet *p){
	...
	/*if(scb->client_ip==GET_SRC_IP(p)->ip32){
	 scb->src_bytes+=p->pkth->pktlen;
	 }else{
	 scb->dst_bytes+=p->pkth->pktlen;
	 }*/
	...
	/*if (IS_IP4(p)) {
		if (sfip_fast_eq4(&p->ip4h->ip_src, &scb->origin_client_ip)
				&& sfip_fast_eq4(&p->ip4h->ip_dst, &scb->origin_server_ip)) {
			scb->src_bytes += p->pkth->pktlen;
		} else if (sfip_fast_eq4(&p->ip4h->ip_src, &scb->origin_server_ip)
				&& sfip_fast_eq4(&p->ip4h->ip_dst, &scb->origin_client_ip)) {
			scb->dst_bytes += p->pkth->pktlen;
		}
	} else {
		if (sfip_fast_eq6(&p->ip6h->ip_src, &scb->origin_client_ip)
				&& sfip_fast_eq6(&p->ip6h->ip_dst, &scb->origin_server_ip)) {
			scb->src_bytes += p->pkth->pktlen;
		} else if (sfip_fast_eq6(&p->ip6h->ip_src, &scb->origin_server_ip)
				&& sfip_fast_eq6(&p->ip6h->ip_dst, &scb->origin_client_ip)) {
			scb->dst_bytes += p->pkth->pktlen;
		}
	}*/
	...
	if (IS_IP4(p)) {
			if (sfip_fast_eq4(&p->ip4h->ip_src, &scb->origin_client_ip)
					&& sfip_fast_eq4(&p->ip4h->ip_dst, &scb->origin_server_ip)) {
				scb->src_bytes += p->dsize;
			} else if (sfip_fast_eq4(&p->ip4h->ip_src, &scb->origin_server_ip)
					&& sfip_fast_eq4(&p->ip4h->ip_dst, &scb->origin_client_ip)) {
				scb->dst_bytes += p->dsize;
			}
		} else {
			if (sfip_fast_eq6(&p->ip6h->ip_src, &scb->origin_client_ip)
					&& sfip_fast_eq6(&p->ip6h->ip_dst, &scb->origin_server_ip)) {
	...
	//printf("\nsrc_bytes=%ld,dst_bytes=%ld,packetSize=%d\n", scb->src_bytes, scb->dst_bytes,p->dsize);
...
static void *getSessionControlBlock( void *sessionCache, Packet *p, SessionKey *key )
{
    ...
    if( getSessionKey( p, key ) )
    {
		...
		if (scb != NULL) {
			...
			scb->last_data_seen1=p ? (p->pkth->ts.tv_sec * (uint64_t) 1000000 + p->pkth->ts.tv_usec) : packet_total_time();// add by zhbit jacky
		}
    }
...
static int deleteSession(void *sessionCache, void *session, char *delete_reason)
{
            ...
            && ((old_mem_in_use - session_mem_in_use ) > prune_log_max))
    {
        ...
        LogMessage("S511: Pruned session from cache that was "
...
static void initSCB(SessionControlBlock *scb,Packet *p){
	...
	scb->last_data_seen1=scb->first_data_seen =
			p ? (p->pkth->ts.tv_sec * (uint64_t) 1000000 + p->pkth->ts.tv_usec) : packet_total_time();
	scb->src_bytes = 0;
	scb->dst_bytes = 0;
	scb->sessionState = SESSION_STATE_INIT;
    //printf("sp=%d,dp=%d\n",p->sp,p->dp);
	if (IS_IP4(p)) {
		scb->origin_client_ip = p->ip4h->ip_src;

		scb->origin_server_ip = p->ip4h->ip_dst;

	} else {
		scb->origin_client_ip = p->ip6h->ip_src;
		scb->origin_server_ip = p->ip6h->ip_dst;
	}
	...
	case IPPROTO_UDP:
	case IPPROTO_TCP:
		scb->origin_client_port = p->sp;
		scb->origin_server_port = p->dp;
		break;
	case IPPROTO_ICMP:
		if (IS_IP4(p)) {
			scb->origin_client_port = p->icmph->type;
			scb->origin_server_port = p->icmph->code;
		} else {
			scb->origin_client_port = p->icmp6h->type;
			scb->origin_server_port = p->icmp6h->code;
		}
		break;
	default:
		scb->origin_client_port = scb->origin_server_port = 0;
		break;
	}
...
static void *createSession(void *sessionCache, Packet *p, const SessionKey *key )
{
    ...
    if (hnode && hnode->data)
    {
        ...
        /*****************zhbit jacky*********************/
        if(GET_IPH_PROTO(p)==IPPROTO_TCP)
           //setTcpStatisticSession(session_cache,scb);
           setTcpSessionStatistics(session_cache,scb);
        else if(GET_IPH_PROTO(p)==IPPROTO_ICMP || GET_IPH_PROTO(p)==IPPROTO_UDP)
        	//setNoTcpStatisticSession(session_cache,scb);
           setNoTcpSessionStatistics(session_cache,scb);
        /*****************zhbit jacky*********************/
...
static void deleteSessionIfClosed( Packet* p )
{
    ...
    if (scb->session_state & STREAM_STATE_CLOSED)
    {
		...
		char *dstP = SnortStrdup(inet_ntoa(&scb->origin_server_ip));
       /* printf("\n***********delete start*************\n");
		printf("srcIp=%s,dstIp=%s,srcPort=%d,dstPort=%d\n", srcP, dstP,
				scb->origin_client_port, scb->origin_server_port);
		 printf("\n***********delete end*************\n");*/
        switch (scb->protocol)
        {
            case IPPROTO_TCP:
                //deleteSession(proto_session_caches[SESSION_PROTO_TCP], scb, "closed normally");  delete by jacky
                //p->ssnptr = NULL;delete by jacky
                break;
            case IPPROTO_UDP:
                //deleteSession(proto_session_caches[SESSION_PROTO_UDP], scb, "closed normally"); delete by jacky
                //p->ssnptr = NULL;delete by jacky
                break;
            case IPPROTO_IP:
            	printf("deleteSessionIfClosed\n");
                deleteSession(proto_session_caches[SESSION_PROTO_IP], scb, "closed normally");
                p->ssnptr = NULL;
                break;
            case IPPROTO_ICMP:
                //deleteSession(proto_session_caches[SESSION_PROTO_ICMP], scb, "closed normally");delete by jacky
                //p->ssnptr = NULL;delete by jacky
                break;
            default:
                break;

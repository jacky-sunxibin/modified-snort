...
static void PrintAllFrag3Frag(FragTracker *ft)
{
	Frag3Frag *idx = NULL;
...
static void Frag3Defrag(Packet *p, void *context)
{
    ...
    /***zhbit jacky start******/
//    printf("sip=%s\n",FragIPToStr(&p->iph->ip_src.s_addr,4));
//    printf("dip=%s\n",FragIPToStr(&p->iph->ip_dst.s_addr,4));
//    printf("frag=%d\n",p->frag_flag);
    /***zhbit jacky end******/
        ...
        Frag3Context *f3context)
{
    ...
    /*PrintFragTracker(ft);
    PrintFragKey(fkey);
    PrintAllFrag3Frag(ft); zhbit jacky*/
    ...
    if(left)
    {
        ...
        if(overlap > 0)
        {
            f3stats.overlaps++;
            ft->overlap_count++;
            p->overlapCount=ft->overlap_count;//zhbit jacky
           // printf("first overlapcount=%d\n",p->overlapCount);//zhbit jacky
            if(frag_end < ft->calculated_size ||
                    ((ft->frag_flags & FRAG_GOT_LAST) &&

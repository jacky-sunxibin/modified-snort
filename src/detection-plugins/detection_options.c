...
#include "sp_wrong_fragment_check.h"  //zhbit jacky
#include "sp_duration_check.h"  //zhbit jacky
#include "sp_session_flag_check.h"  //zhbit jacky
#include "sp_srcBytes_check.h"  //zhbit jacky
#include "sp_dstBytes_check.h"  //zhbit jacky
#include "sp_twoSecondsSameDstHostSessions_check.h"  //zhbit jacky
#include "sp_twoSecondsSameDstHostServiceRate_check.h" //zhbit jacky
#include "sp_twoSecondsSameDstHostSynErrorRate_check.h" //zhbit jacky
#include "sp_twoSecondsSameDstServiceSynErrorRate_check.h" //zhbit jacky
#include "sp_twoSecondsSameDstHostDiffServiceRate_check.h" //zhbit jacky

#include "sp_twoSecondsSameDstHostRejErrorRate_check.h" //zhbit jacky
#include "sp_twoSecondsSameDstServiceDiffDstHostRate_check.h" //zhbit jacky
#include "sp_twoSecondsSameDstServiceRejErrorRate_check.h" //zhbit jacky
#include "sp_twoSecondsSameDstServiceSessions_check.h" //zhbit jacky


#include "sp_before100SameDstHostSynErrorRate_check.h"  //zhbit jacky
#include "sp_before100SameDstHostServiceSynErrorRate_check.h" //zhbit jacky
#include "sp_before100SameDstHostRejErrorRate_check.h" //zhbit jacky
#include "sp_before100SameDstHostSessions_check.h" //zhbit jacky
#include "sp_before100SameDstHostServiceSessions_check.h" //zhbit jacky
#include "sp_before100SameDstHostServiceRate_check.h" //zhbit jacky
//#include "sp_before100SameDstHostRejErrorRate_check.h" //zhbit jacky

#include "sp_before100SameDstHostSourcePortRate_check.h" //zhbit jacky
#include "sp_before100SameDstHostServiceRejErrorRate_check.h" //zhbit jacky
#include "sp_before100SameDstHostServiceDiffSIPRate_check.h" //zhbit jacky
#include "sp_before100SameDstHostDiffServiceRate_check.h" //zhbit jacky
...
uint32_t detection_option_hash_func(SFHASHFCN *p, unsigned char *k, int n)
{
    ...
    switch (key->option_type)
    {
        ...
        case RULE_OPTION_TYPE_IP_SAME:
            ...
            break;
        case RULE_OPTION_TYPE_WRONG_FRAGMENT: //zhbit jacky
            hash = WrongFragmentCheckHash(key->option_data);
            break;
        case RULE_OPTION_TYPE_SESSION_FLAG: //zhbit jacky
            hash = SessionFlagCheckHash(key->option_data);
            break;
	    case RULE_OPTION_TYPE_DURATION: //zhbit jacky
		    hash = DurationCheckHash(key->option_data);
		    break;
	    case RULE_OPTION_TYPE_SRCBYTES: //zhbit jacky
		    hash = SrcBytesCheckHash(key->option_data);
		    break;
	    case RULE_OPTION_TYPE_DSTBYTES: //zhbit jacky
	         hash = DstBytesCheckHash(key->option_data);
	         break;
	    case RULE_OPTION_TYPE_TWOSECONDSSAMEDSTHOSTSESSIONS: //zhbit jacky
	    	hash = TwoSecondsSameDstHostSessionsCheckHash(key->option_data);
	    	break;
	    case RULE_OPTION_TYPE_TWOSECONDSSAMEDSTHOSTSERVICERATE: //zhbit jacky
	    	 hash = TwoSecondsSameDstHostServiceRateCheckHash(key->option_data);
	    	 break;
	    case RULE_OPTION_TYPE_TWOSECONDSSAMEDSTSERVICESYNERRORRATE: //zhbit jacky
	    	 hash = TwoSecondsSameDstServiceSynErrorRateCheckHash(key->option_data);
	    	 break;
	    case RULE_OPTION_TYPE_TWOSECONDSSAMEDSTHOSTSYNERRORRATE: //zhbit jacky
	    	 hash = TwoSecondsSameDstHostSynErrorRateCheckHash(key->option_data);
	    	 break;
	    case RULE_OPTION_TYPE_TWOSECONDSSAMEDSTHOSTDIFFSERVICERATE: //zhbit jacky
	    	 hash = TwoSecondsSameDstHostDiffServiceRateCheckHash(key->option_data);
	    	 break;
	    case RULE_OPTION_TYPE_TWOSECONDSSAMEDSTHOSTREJERRORRATE: //zhbit jacky
	   	     hash = TwoSecondsSameDstHostRejErrorRateCheckHash(key->option_data);
	   	     break;
	    case RULE_OPTION_TYPE_TWOSECONDSSAMEDSTSERVICEDIFFDSTHOSTRATE: //zhbit jacky
	    	 hash = TwoSecondsSameDstServiceDiffDstHostRateCheckHash(key->option_data);
	    	 break;
	    case RULE_OPTION_TYPE_TWOSECONDSSAMEDSTSERVICEREJERRORRATE: //zhbit jacky
	    	 hash = TwoSecondsSameDstServiceRejErrorRateCheckHash(key->option_data);
	    	 break;
	    case RULE_OPTION_TYPE_TWOSECONDSSAMEDSTSERVICESESSIONS: //zhbit jacky
	    	 hash = TwoSecondsSameDstServiceSessionsCheckHash(key->option_data);
	    	 break;

	    case RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTSYNERRORRATE: //zhbit jacky
	   	    hash = Before100SameDstHostSynErrorRateCheckHash(key->option_data);
	   	    break;
	    case RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTSERVICESYNERRORRATE: //zhbit jacky
	    	hash = Before100SameDstHostServiceSynErrorRateCheckHash(key->option_data);
	    	break;
	    case RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTREJERRORRATE: //zhbit jacky
	    	hash = Before100SameDstHostRejErrorRateCheckHash(key->option_data);
	    	break;
	    case RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTSESSIONS: //zhbit jacky
	    	 hash = Before100SameDstHostSessionsCheckHash(key->option_data);
	    	 break;
	    case RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTSERVICESESSIONS: //zhbit jacky
	    	 hash = Before100SameDstHostServiceSessionsCheckHash(key->option_data);
	    	 break;
	    case RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTSERVICERATE: //zhbit jacky
	    	 hash = Before100SameDstHostServiceRateCheckHash(key->option_data);
	    	 break;
	    case RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTSOURCEPORTRATE: //zhbit jacky
	    	 hash = Before100SameDstHostSourcePortRateCheckHash(key->option_data);
	    	 break;
	    case RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTSERVICEREJERRORRATE: //zhbit jacky
	    	 hash = Before100SameDstHostServiceRejErrorRateCheckHash(key->option_data);
	    	 break;
	    case RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTSERVICEDIFFSIPRATE: //zhbit jacky
	    	 hash = Before100SameDstHostServiceDiffSIPRateCheckHash(key->option_data);
	    	 break;
	    case RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTDIFFSERVICERATE: //zhbit jacky
	    	 hash = Before100SameDstHostDiffServiceRateCheckHash(key->option_data);
	    	 break;
...
int detection_option_key_compare_func(const void *k1, const void *k2, size_t n)
{
    ...
    switch (key1->option_type)
    {
        ...
        case RULE_OPTION_TYPE_IP_SAME:
            ...
            break;
        case RULE_OPTION_TYPE_WRONG_FRAGMENT: //zhbit jacky
            ret = WrongFragmentCheckCompare(key1->option_data, key2->option_data);
             break;
        case RULE_OPTION_TYPE_SESSION_FLAG: //zhbit jacky
             ret = SessionFlagCheckCompare(key1->option_data, key2->option_data);
             break;
        case RULE_OPTION_TYPE_DURATION: //zhbit jacky
             ret = DurationCheckCompare(key1->option_data, key2->option_data);
             break;
        case RULE_OPTION_TYPE_SRCBYTES: //zhbit jacky
             ret = SrcBytesCheckCompare(key1->option_data, key2->option_data);
             break;
        case RULE_OPTION_TYPE_DSTBYTES: //zhbit jacky
             ret = DstBytesCheckCompare(key1->option_data, key2->option_data);
             break;
        case RULE_OPTION_TYPE_TWOSECONDSSAMEDSTHOSTSESSIONS: //zhbit jacky
             ret = TwoSecondsSameDstHostSessionsCheckCompare(key1->option_data, key2->option_data);
             break;
        case RULE_OPTION_TYPE_TWOSECONDSSAMEDSTHOSTSERVICERATE: //zhbit jacky
             ret = TwoSecondsSameDstHostServiceRateCheckCompare(key1->option_data, key2->option_data);
             break;
        case RULE_OPTION_TYPE_TWOSECONDSSAMEDSTHOSTSYNERRORRATE: //zhbit jacky
             ret = TwoSecondsSameDstHostSynErrorRateCheckCompare(key1->option_data, key2->option_data);
             break;
        case RULE_OPTION_TYPE_TWOSECONDSSAMEDSTHOSTDIFFSERVICERATE: //zhbit jacky
             ret = TwoSecondsSameDstHostDiffServiceRateCheckCompare(key1->option_data, key2->option_data);
             break;
        case RULE_OPTION_TYPE_TWOSECONDSSAMEDSTSERVICESYNERRORRATE: //zhbit jacky
             ret = TwoSecondsSameDstServiceSynErrorRateCheckCompare(key1->option_data, key2->option_data);
             break;
        case RULE_OPTION_TYPE_TWOSECONDSSAMEDSTHOSTREJERRORRATE: //zhbit jacky
             ret = TwoSecondsSameDstHostRejErrorRateCheckCompare(key1->option_data, key2->option_data);
             break;
        case RULE_OPTION_TYPE_TWOSECONDSSAMEDSTSERVICEDIFFDSTHOSTRATE: //zhbit jacky
             ret = TwoSecondsSameDstServiceDiffDstHostRateCheckCompare(key1->option_data, key2->option_data);
             break;
        case RULE_OPTION_TYPE_TWOSECONDSSAMEDSTSERVICEREJERRORRATE: //zhbit jacky
             ret = TwoSecondsSameDstServiceRejErrorRateCheckCompare(key1->option_data, key2->option_data);
             break;
        case RULE_OPTION_TYPE_TWOSECONDSSAMEDSTSERVICESESSIONS: //zhbit jacky
             ret = TwoSecondsSameDstServiceSessionsCheckCompare(key1->option_data, key2->option_data);
             break;



        case RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTSYNERRORRATE: //zhbit jacky
             ret = Before100SameDstHostSynErrorRateCheckCompare(key1->option_data, key2->option_data);
             break;
        case RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTSERVICESYNERRORRATE: //zhbit jacky
             ret = Before100SameDstHostServiceSynErrorRateCheckCompare(key1->option_data, key2->option_data);
             break;
        case RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTREJERRORRATE: //zhbit jacky
             ret = Before100SameDstHostRejErrorRateCheckCompare(key1->option_data, key2->option_data);
             break;
        case RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTSESSIONS: //zhbit jacky
             ret = Before100SameDstHostSessionsCheckCompare(key1->option_data, key2->option_data);
             break;
        case RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTSERVICESESSIONS: //zhbit jacky
             ret = Before100SameDstHostServiceSessionsCheckCompare(key1->option_data, key2->option_data);
             break;
        case RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTSERVICERATE: //zhbit jacky
             ret = Before100SameDstHostServiceRateCheckCompare(key1->option_data, key2->option_data);
             break;
        case RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTSOURCEPORTRATE: //zhbit jacky
             ret = Before100SameDstHostSourcePortRateCheckCompare(key1->option_data, key2->option_data);
             break;
        case RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTSERVICEREJERRORRATE: //zhbit jacky
             ret = Before100SameDstHostServiceRejErrorRateCheckCompare(key1->option_data, key2->option_data);
             break;
        case RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTSERVICEDIFFSIPRATE: //zhbit jacky
             ret = Before100SameDstHostServiceDiffSIPRateCheckCompare(key1->option_data, key2->option_data);
             break;
        case RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTDIFFSERVICERATE: //zhbit jacky
             ret = Before100SameDstHostDiffServiceRateCheckCompare(key1->option_data, key2->option_data);
             break;
...
char *option_type_str[] =
{
    ...
    "RULE_OPTION_TYPE_WRONG_FRAGMENT",//zhbit jacky
    "RULE_OPTION_TYPE_DURATION",//zhbit jacky
    "RULE_OPTION_TYPE_SESSION_FLAG",//zhbit jacky
    "RULE_OPTION_TYPE_SRCBYTES",//zhbit jacky
    "RULE_OPTION_TYPE_DSTBYTES",//zhbit jacky
    "RULE_OPTION_TYPE_TWOSECONDSSAMEDSTHOSTSESSIONS", //zhbit jacky
    "RULE_OPTION_TYPE_TWOSECONDSSAMEDSTHOSTSERVICERATE", //zhbit jacky
    "RULE_OPTION_TYPE_TWOSECONDSSAMEDSTHOSTSYNERRORRATE", //zhbit jacky
    "RULE_OPTION_TYPE_TWOSECONDSSAMEDSTSERVICESYNERRORRATE", //zhbit jacky
    "RULE_OPTION_TYPE_TWOSECONDSSAMEDSTHOSTDIFFSERVICERATE", //zhbit jacky
    "RULE_OPTION_TYPE_TWOSECONDSSAMEDSTHOSTREJERRORRATE", //zhbit jacky
    "RULE_OPTION_TYPE_TWOSECONDSSAMEDSTSERVICEDIFFDSTHOSTRATE", //zhbit jacky
    "RULE_OPTION_TYPE_TWOSECONDSSAMEDSTSERVICEREJERRORRATE", //zhbit jacky
    "RULE_OPTION_TYPE_TWOSECONDSSAMEDSTSERVICESESSIONS", //zhbit jacky

    "RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTSYNERRORRATE", //zhbit jacky
    "RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTSERVICESYNERRORRATE", //zhbit jacky
    "RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTREJERRORRATE", //zhbit jacky
    "RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTSESSIONS", //zhbit jacky
    "RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTSERVICESESSIONS", //zhbit jacky
    "RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTSERVICERATE", //zhbit jacky

    "RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTSOURCEPORTRATE", //zhbit jacky
    "RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTSERVICEREJERRORRATE", //zhbit jacky
    "RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTSERVICEDIFFSIPRATE", //zhbit jacky
    "RULE_OPTION_TYPE_BEFORE100SAMEDSTHOSTDIFFSERVICERATE", //zhbit jacky
